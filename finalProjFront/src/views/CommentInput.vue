<template>
      ...
      <div class="comment-attachments mt-2 p-2 border rounded" @dragover.prevent @drop.prevent="handleDrop">
        ...
        <div class="d-flex flex-wrap gap-2 mt-2">
          <div v-for="(f, idx) in files" :key="idx">
            <img :src="f.preview" style="width: 80px; height: 80px; object-fit: cover; border: 1px solid #ccc;" class="rounded" />
          </div>
        </div>
      </div>
  ...
  async function submit() {
    ...
    try {
      ...
      const res = await axios.post(`http://localhost:8080/TicketComment`, commentPayload)
      ...
      if (files.value.length > 0) {
        ...
        await axios.post(`http://localhost:8080/ticket-attachment/upload/base64/multiple`, base64Files)
      }
