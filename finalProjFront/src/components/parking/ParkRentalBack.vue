<template>
    <div :class="{ 'dark-mode': isDarkMode }" class="container mt-4" style="max-width: 1000px;">
        <div class="w-60 position-relative" style="margin-left: calc(-50vw + 50%); width: 60vw;">
            <BannerImage
                :imageSrc="bannerImg"
                heading="車位承租管理後台"
                subtext="管理員可查詢、審核、編輯與管理社區所有車位承租紀錄，並進行進階搜尋、狀態篩選與詳細資料維護。"
                textAlign="left"
            />
        </div>
        <div class="d-flex justify-content-between align-items-center mb-3">
            <div class="tag-style px-4 py-2" :class="{ 'dark-mode': isDarkMode }">
                <h4 class="mb-0 fw-bold text-primary section-title">社區承租紀錄</h4>
            </div>
        </div>
    ...
    <div class="table-responsive shadow-sm rounded mb-4">
        <table v-if="availableSlots.length" class="table table-hover table-borderless align-middle mb-0">
            <thead>
                <tr>
                    <th>流水號</th>
                    <th>車位編號</th>
            ...
            <tbody>
                <tr v-for="record in filteredRecords" :key="record.id">
                    ...
                    <td>
                        <button class="btn btn-primary btn-sm" data-bs-toggle="modal" data-bs-target="#slotDetailModal"
        ...
        <p v-else class="text-center my-4 text-secondary">尚無可承租車位。</p>
    </div>
    ...
    <div class="modal fade" id="slotDetailModal" tabindex="-1">
        <div class="modal-dialog modal-lg">
            <div class="modal-content p-4">
                <h5 class="fw-bold mb-3">{{ isAddMode ? '新增承租紀錄' : '承租紀錄詳情' }}</h5>
                ...
                <div v-if="selectedRecord">
                    <div class="row g-3">
                        ...
                        <!-- 繳費狀態 -->
                        <div class="col-md-6 mb-2" v-if="!isAddMode">
...
<style>
/* 移除原本style，統一交由全域dark-mode樣式控制 */
.dark-mode .card.card-body {
  background: var(--color-bg-dark-secondary, #232a36) !important;
  color: var(--color-text-dark, #f1f1f1) !important;
  border: 1px solid var(--color-border-dark, #2c3440) !important;
  box-shadow: 0 2px 12px rgba(40,60,100,0.10);
}
