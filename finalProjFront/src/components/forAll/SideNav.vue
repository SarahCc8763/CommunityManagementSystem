<template>
  <aside class="bg-light border-end p-3" style="min-width: 240px;">
    ...
    <div v-if="UserStore.isAuthenticated" class="mb-4">
      <div class="card user-profile-card border-0 shadow-sm p-3 position-relative" data-bs-toggle="dropdown"
        aria-expanded="false" style="cursor: pointer;">
        <!-- 頭像與名稱 -->
        <div class="d-flex align-items-center">
          <img :src="UserStore.avatarUrl" alt="頭像" class="rounded-circle me-3 border" width="64" height="64" />
          <div>
        ...
        <li v-for="(item, idx) in menuItems" :key="idx" class="mb-2">
          ...
          <div v-if="item.children">
            <div
              ...
              style="cursor:pointer;" @click="toggle(idx)">
              <router-link :to="item.link || '#'" class="flex-grow-1 text-dark text-decoration-none" @click.stop>
                {{ item.label }}
              </router-link>
